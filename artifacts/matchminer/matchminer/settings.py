...
try:
    ...
    load_dotenv(BASE_DIR / ".env")  # artifacts/matchminer/.env
...
def _allowed_hosts():
    """Return a host allow-list that fits Replit and ordinary deployments.

    Production should not accept arbitrary Host headers because absolute URLs are
    generated in a few places (schedule examples, run event URLs). Replit domains
    are dynamic, so allow their known suffixes plus any explicit custom domains.
    Additional hosts can be supplied with DJANGO_ALLOWED_HOSTS.
    """
    hosts = {
        "localhost",
        "127.0.0.1",
    ...
    hosts.update(_csv_env("DJANGO_ALLOWED_HOSTS"))
    for domain in _csv_env("REPLIT_DOMAINS"):
        if "://" in domain:
...
TRUSTED_PROXY_IPS = _csv_env("DJANGO_TRUSTED_PROXY_IPS")
...
TEMPLATES = [
    {
        ...
        "OPTIONS": {
            "context_processors": [
                ...
                "accounts.context_processors.nav_scrapers",
            ],
        },
    },
]
...
# SportRadar Tennis API credentials. The scraper first uses the per-scraper
# Settings secret, then this env fallback. The key is sent in the x-api-key
# header and is never placed in request URLs.
