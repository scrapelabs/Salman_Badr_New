...
DEFAULT_TICKET_ASSIGNEE = "khemiri"
...
def _default_ticket_assignee():
    return User.objects.filter(
        username__iexact=DEFAULT_TICKET_ASSIGNEE,
        is_active=True,
    ).first()
...
def _notification_history_redirect():
    return redirect(reverse("overview") + "#notification-history")
...
def notification_delete(request, pk):
    ...
    return _notification_history_redirect()
...
def notifications_bulk_delete(request):
    ...
    deleted, _ = Notification.objects.filter(recipient=request.user, pk__in=ids).delete()
    if deleted:
        suffix = "" if deleted == 1 else "s"
        messages.success(request, f"Deleted {deleted} notification{suffix}.")
    else:
