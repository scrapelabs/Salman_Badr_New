...
_PROMOTED_PLAYER_NOTE_RE = re.compile(
    r"\s*\([^)]*(?:Tõstetud|TÃµstetud|Etapiks|Üles|Ãœles)[^)]*\)",
    re.IGNORECASE,
)
...
def _clean_name(name):
    """Drop seed/promoted-player annotations from a player name."""
    name = re.sub(r"\s*\[[^\]]+\]\s*$", "", name or "").strip()
    name = _PROMOTED_PLAYER_NOTE_RE.sub("", name)
...
def _tournament_player_links(client, tid):
    ...
    if resp is not None and 200 <= resp.status_code < 300:
        ...
        for a in sel.xpath(
            ...
            href = _pf(a, "./@href")
            if href:
                links.append((_clean_name(name), urljoin(BASE, href)))
    return links
...
def _resolve(client, side, players_db, url_to_id, page_cache):
    ...
    name = _clean_name(side.get("name", "") or "")
    if not url:
...
def _scrape_tournament(client, tournament, claude_keys):
    ...
    # Stage B: players table keyed by third_party_id. Estonia can list the same
    # player twice when they were promoted to the next stage; the promoted entry
    # has no licence id, so alias it back to the real same-name player when one
    # exists in the tournament player list.
    pending_players = []
    real_id_by_name = {}
    url_to_id = {}
    for listing_name, purl in links:
        listing_name = _clean_name(listing_name)
        sel = _get_sel(client, purl, page_cache)
        if sel is None:
            continue
        tpid = _player_id(client, sel, listing_name, page_cache)
        url_to_id[purl] = tpid
        pending_players.append((listing_name, purl, tpid, sel))
        if "local_" not in (tpid or ""):
            real_id_by_name.setdefault(listing_name, tpid)

    players_db = {}
    for listing_name, purl, tpid, sel in pending_players:
        if "local_" in (tpid or ""):
            real_tpid = real_id_by_name.get(listing_name)
            if real_tpid:
                tpid = real_tpid
                url_to_id[purl] = real_tpid
