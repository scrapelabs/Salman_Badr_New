"""Uruguay Results (AUT / uruguay.tenisintegrado.com) scraper.

Ports the production ``uruguay_results`` spider onto MatchMiner's shared HTTP
client (:mod:`accounts.live_scrapers._http`) + telemetry. Input is a season
**year + month** (month ``0`` = whole year). The flow mirrors its Brazilian
sibling but against the Uruguayan ``tenisintegrado`` instance:

1. POST the ``new_torneio/index2`` search, then follow the two top-level
   sections (**Menores** and **Profesional**); for each, walk the month tabs
   and collect each month's tournament links;
...
Like the Brazil port this is **deterministic and AI-free**: the original
optionally pretty-formatted new player names via Claude, but that only affected
display casing (gender always comes from the draw), so it is dropped — names are
emitted as scraped (normalised to ``"Last, First"`` unless already comma-form).

The production spider also uploaded its CSV to Azure Blob storage; that output
side-channel is dropped — MatchMiner persists rows to the DB / downloadable CSV.
...
BASE = "https://uruguay.tenisintegrado.com"
INDEX2_URL = f"{BASE}/new_torneio/index2"
PAINEL_URL = f"{BASE}/torneio_painel_jogo"

# Items CSV columns — identical 60-col tenisintegrado items schema as Brazil
...
# ======================================================================
# Match-block parser (ported from the production Uruguay Parser; logger / DB /
# Claude dependencies removed — pure HTML -> dict).
# ======================================================================
class _MatchParser:
    BALL_TYPE = "Yellow"
    COUNTRY = "Uruguay"
    ...
    # game-top header layouts (kept verbatim from source — the platform's
    # markup keeps Portuguese round tokens even on the Uruguay instance):
    ...
    def _last_first(cls, full_name):
        ...
        if not name:
            return ""
        # If the name already contains a comma, assume it's already in
        # "Last, First Middle" form — just trim any stray trailing comma /
        # whitespace and return as-is (don't reorder it again).
        if "," in name:
    ...
    def _extract_tournament(self, selector):
        ...
        if share_url:
            ...
            if id_torneio:
                # NB: the source fallback hard-codes the *Brazilian* host here
                # (a copy-paste slip); use the Uruguay base so the emitted URL
                # is correct.
                self.tournament_url = (
    ...
    def _extract_draw(self, selector):
        ...
        for txt in selector.xpath(
            ...
            txt = self._clean(txt)
            if "Singles" in txt or "Dobles" in txt:
                name = txt
                break
        if not name:
            opt = selector.xpath(
        ...
        low = name.lower()
        self.draw_gender = (
            "Male" if "caballeros" in low else ("Female" if "damas" in low else "")
        )
        self.draw_team_type = (
            "Doubles" if "dobles" in low else ("Singles" if "singles" in low else "")
        )
        ...
        try:
            ...
            if winner_name_raw:
                ...
                for idx, players in enumerate((row_a_players, row_b_players)):
                    if any(
                        self._strip_markers(p[0]).lower() == target
                        or self._strip_markers(p[0]).lower() in target
                        or target in self._strip_markers(p[0]).lower()
                        for p in players
                    ):
                        winner_idx = idx
                        break
            ...
            match_data = {
                # The source recomputes a match_id from the header but then
                # forces it empty before persisting; emit it empty to match.
                "match_id": "",
...
def _discover_tournaments(client, year, month, log):
    ...
    # The index2 search response carries the two top-level sections we want
    # (Menores + Profesional) as nav tabs. We follow each section's *rendered*
    # href as-is — exactly like the source (and the brazil_results sibling).
    # Note: the site always renders the section/month tabs for the *current*
    # season regardless of the POSTed ``ano``, so the year input is effectively
    # cosmetic here (faithful to the source); the month tabs do filter.
    sel = Selector(text=resp.text)
    index_links = []
    for label in ("Menores", "Profesional"):
        href = sel.xpath(
            f'//ul[contains(@class,"nav-tabs")]//li/a[contains(text(), "{label}")]/@href'
        ).get()
        if href:
            index_links.append(urljoin(BASE + "/", href.strip()))
    log("INFO", f"\U0001f5c2\ufe0f {len(index_links)} section page(s) found")

    tournaments = []
    seen = set()
    for index_link in index_links:
        isel = client.get_selector(index_link)
        if isel is None:
            continue
        # Month tabs: anchor on the "Feb" tab (the source's literal ``spam``
        # typo matches the site markup), then take every enabled month link;
        # keep only the requested month unless month == 0 (whole season).
        for d1 in isel.xpath(
            '//ul[contains(@class,"nav-tabs")]//li[a/spam[text()="Feb"]]'
            ...
            msel = client.get_selector(month_url)
            if msel is None:
                continue
            for a in msel.xpath(
...
def _ascii_lower(text):
    return "".join(
        ch for ch in unicodedata.normalize("NFKD", text or "") if ord(ch) < 128
    ).lower()


def _empty_panel_reason(sel):
    text = _ascii_lower(" ".join(sel.xpath("//text()").getall()))
    if "no existen juegos calculados" in text or "cuadros aun no estan listos" in text:
        return "games are not calculated yet"
    return ""
...
def _parse_category(client, tournament_url, sel, log=None, empty_reasons=None):
    ...
    if not id_categorias and not id_parametros and not sel.xpath('//div[@class="game"]'):
        reason = _empty_panel_reason(sel) or "no category/game controls found"
        if empty_reasons is not None:
            empty_reasons.append(reason)
        if log:
            log("INFO", f"   \u2139\ufe0f {tournament_url}: {reason}")
        return out
...
def _scrape_tournament(client, tournament_url, log=None, empty_reasons=None):
    ...
    if warm is None or not (200 <= warm.status_code < 300):
        return []
    # The bare GET /torneio_painel_jogo returns whatever tournament the cffi
    # session last touched, so iterating multiple tournaments can silently
    # re-scrape the same one. Fetch the bracket page with an explicit
    # /index/{tournament_id} when the URL carries an id so it's deterministic.
    ...
    return _parse_category(
        client, tournament_url, sel, log=log, empty_reasons=empty_reasons
    )


def run(run_obj, log):
    """Execute the Uruguay Results scrape. Returns the standard 5-tuple."""
    tele = Telemetry()
    ...
    log(
        "INFO",
        f"\U0001f3be Uruguay Results (AUT) starting \u2014 year={year}, "
    ...
    def process(tournament_url):
        ...
        try:
            rows = _scrape_tournament(
                client, tournament_url, log=log, empty_reasons=empty_reasons
            )
            if not rows:
                log("INFO", f"   \u2139\ufe0f no match rows available for {tournament_url}")
            for row in rows:
                key = (
                    tournament_url,
                    row.get("match_id", ""),
                ...
                log(
                    ...
                    f"@ {row.get('tournament_name') or 'Uruguay'}",
                )
    ...
    if row_count:
        ...
        log(
            "INFO",
            "\u2139\ufe0f No calculated match rows are available yet; treating as a healthy empty run.",
        )
        status = Run.Status.SUCCESS
    else:
