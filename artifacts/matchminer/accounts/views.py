...
def _default_range_days(spec):
    """Blank date-range default for scheduled/example runs."""
    value = getattr(spec, "default_range_days", 0) or DEFAULT_RANGE_DAYS
    return max(0, min(value, MAX_RANGE_DAYS))
...
def _run_params_link(run):
    """Clickable URL for URL-input jobs; empty for ordinary labels/windows."""
    params = run.params if isinstance(run.params, dict) else {}
    url = _safe_http_url(params.get("tournament_url"))
    return url or _safe_http_url(run.tournament or "")
...
def _threads_running():
    ...
    """
    threads = list(
        Scraper.objects.filter(runs__status=Run.Status.RUNNING)
        .distinct()
        .values_list("threads", flat=True)
    )
    workers = [
        max(Scraper.THREADS_MIN, min(value or Scraper.THREADS_DEFAULT, Scraper.THREADS_MAX))
        for value in threads
    ]
    return sum(workers), len(workers)


def _recent_runs(limit=OVERVIEW_RECENT_RUN_LIMIT):
    """Latest non-queued jobs for the Overview table.

    Keep this bounded: Overview and its live poll should never materialise the
    whole historical run table just to render a small dashboard widget.
    """
    qs = (
        Run.objects.exclude(status=Run.Status.QUEUED)
        .select_related("scraper")
        .defer(*RUN_BLOB_FIELDS)
        .order_by("-started_at")
    )
    ...
    for r in runs:
        ...
        r.tournament_link = _run_params_link(r)
    return runs


def _notification_history(user):
    return (
        Notification.objects.filter(recipient=user)
        .select_related("actor", "ticket", "ticket__scraper")
...
def _runs_today_count():
    ...
    return (
        Run.objects.exclude(status=Run.Status.QUEUED)
        .filter(started_at__gte=today_start)
        .count()
    )


def _run_poll_queryset(qs):
    """Small Run projection for hot poll endpoints.

    Pollers update every few seconds and only need metadata plus yes/no export
    availability. Keep giant text blobs out of Python memory; the Length()
    annotations let the database answer availability without transferring the
    blob payloads.
    """
    return qs.only(*RUN_POLL_FIELDS).annotate(
        _csv_len=Length("csv_data"),
        _requests_len=Length("requests_csv"),
        _errors_len=Length("errors_csv"),
    )


def _run_output_flags(run):
    return {
        "has_csv": bool((run.output_size_bytes or 0) or getattr(run, "_csv_len", 0)),
        "has_requests": bool(getattr(run, "_requests_len", 0)),
        "has_errors": bool(getattr(run, "_errors_len", 0)),
    }
...
def _run_brief(run):
    return {
        ...
        "tournament_url": _run_params_link(run),
...
def _queued_jobs(limit=5):
    ...
    for r in jobs:
        ...
        r.params_link = _run_params_link(r)
    return jobs
...
def _queued_brief(run):
    return {
        ...
        "params_url": _run_params_link(run),
        "queued_human": f"{timesince(run.created_at)} ago",
        "detail_url": f"{reverse('scraper_detail', args=[run.scraper.slug])}?tab=batch",
    }


def _batch_jobs_queryset(scraper):
    """Jobs table ordering: running, queued FIFO, then finished newest first."""
    return (
        scraper.runs.annotate(
            _bucket=Case(
...
def _jobs_table_signature(runs):
    """Stable fingerprint for structural Batch table changes, not progress ticks."""
    return ",".join(
        f"{r.uuid}:{r.status}:{r.started_at.isoformat() if r.started_at else ''}:"
...
def _reap_stale_runs(scraper=None):
    ...
    running = (
        Run.objects.filter(status=Run.Status.RUNNING).defer(*RUN_BLOB_FIELDS)
        if scraper is None
        else scraper.runs.filter(status=Run.Status.RUNNING).defer(*RUN_BLOB_FIELDS)
    )
    for run in running:
...
def _is_trusted_proxy(remote_addr):
    try:
        remote_ip = ipaddress.ip_address(remote_addr or "")
    except ValueError:
        return False
    for raw_net in getattr(settings, "TRUSTED_PROXY_IPS", ()):  # exact IP or CIDR
        try:
            if remote_ip in ipaddress.ip_network(str(raw_net), strict=False):
                return True
        except ValueError:
            continue
    return False
...
def _client_ip(request):
    ...
    forwarded = (request.META.get("HTTP_X_FORWARDED_FOR") or "").split(",", 1)[0]
    if forwarded.strip() and _is_trusted_proxy(remote):
        return forwarded.strip().lower()
    return remote.lower()
...
def _login_throttle_keys(request, username):
    ...
    account_name = (username or "").strip().lower() or "<blank>"
    account_digest = hashlib.sha256(account_name.encode("utf-8")).hexdigest()
    return f"login-fail-account:{account_digest}", f"login-fail-ip:{ip_digest}"


def _record_login_failure(account_key, ip_key):
...
def login_view(request):
    ...
    if request.method == "POST":
        ...
        account_key, ip_key = _login_throttle_keys(request, username)
        if (
            cache.get(account_key, 0) >= LOGIN_ACCOUNT_FAILURE_LIMIT
            or cache.get(ip_key, 0) >= LOGIN_IP_FAILURE_LIMIT
        ):
        ...
        if user is not None:
            ...
            login(request, user)
            return redirect("overview")
        _record_login_failure(account_key, ip_key)
...
@login_required
def overview_notifications_view(request):
    """Small, paged notification-history fragment loaded after Overview paints."""
    paginator = Paginator(_notification_history(request.user), OVERVIEW_NOTIFICATION_PAGE_SIZE)
    page_obj = paginator.get_page(request.GET.get("page") or 1)
    html = render_to_string(
        "partials/notification_history.html",
        {
            "notifications": page_obj,
            "notification_page": page_obj,
...
def _run_form_ctx(s, spec, ctx):
    ...
    ctx["default_date_from"] = (
        today - timedelta(days=_default_range_days(spec))
    ).isoformat()
    ...
    ctx["default_bi_weekly_on"] = spec.bi_weekly_default_on
...
def scraper_detail_view(request, slug):
    ...
    if tab == "real-time":
        ...
        jobs_qs = _batch_jobs_queryset(s)
        ...
        ctx["jobs_table_sig"] = _jobs_table_signature(page_obj.object_list)
        ...
        qa_tickets = list(
            s.tickets.select_related("created_by", "assignee")
            .annotate(n_comments=Count("comments"))
            .order_by("-created_at", "-pk")
        )
        ctx["qa_tickets"] = qa_tickets
        ctx["qa_ticket_total"] = len(qa_tickets)
    elif tab == "data":
        ...
        sched_defaults = {
            "year": default_year,
            "month": 0,
            "date_from": (today - timedelta(days=_default_range_days(spec))).isoformat(),
        ...
        ctx["workflow_yaml"] = _github_workflow_yaml(
            ...
            feed_api_key=spec.feed_api_key,
            feed_gender=spec.feed_gender,
            default_range_days=_default_range_days(spec),
...
def validate_run_params(spec, data, *, webhook=False):
    ...
    if kind in (registry.INPUT_DATE_RANGE, registry.INPUT_DATE_RANGE_OR_URL):
        ...
        if bw_on:
            ...
            start = end - timedelta(days=_default_range_days(spec))
        else:
...
def run_events_view(request, slug, run_uuid):
    run = get_object_or_404(
        _run_poll_queryset(Run.objects.filter(scraper__slug=slug)), uuid=run_uuid
    )
    ...
    output_flags = _run_output_flags(run)
    return JsonResponse(
        {
            "status": run.status,
            ...
            "size_label": run.size_label,
            "duration_label": run.duration_label,
            "has_csv": output_flags["has_csv"],
            "has_requests": output_flags["has_requests"],
            "has_errors": output_flags["has_errors"],
        }
    )
...
def queue_events_view(request, slug):
    ...
    running = (
        s.runs.filter(status=Run.Status.RUNNING)
        .only("uuid", "status", "started_at")
        .order_by("-started_at")
        .first()
    )
    queued_uuids = list(
    ...
    jobs = {}
    if ids:
        for r in _run_poll_queryset(s.runs.filter(uuid__in=ids)):
            output_flags = _run_output_flags(r)
            jobs[str(r.uuid)] = {
                "status": r.status,
    ...
    table_page = request.GET.get("page") or "1"
    table_page_obj = Paginator(_batch_jobs_queryset(s), JOBS_PER_PAGE).get_page(
        table_page
    )
    ...
    return JsonResponse(
        {
            ...
            "table_sig": _jobs_table_signature(table_page_obj.object_list),
            "jobs": jobs,
        }
    )
