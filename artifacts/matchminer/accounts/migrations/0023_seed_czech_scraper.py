"""Seed the Czech (cesky-tenis.cz) scraper (idempotent).
...
SCRAPERS = [
    {
        ...
        "name": "Czech Scraper",
        "tour": "CTS",
        "domain": "cesky-tenis.cz",
        "vendor_url": "https://cesky-tenis.cz",
        "description": (
            "Czech national tennis results from cesky-tenis.cz. Walks each "
            "age-group's season listings, finds the men's and women's draws in "
            "the run window, then scrapes each draw's registration list and "
            "result brackets. Input is a date range or a single tournament URL."
        ),
        "returns": "CSV",
        "tournaments": ["Czech Tennis"],
        "mode": "production",
...
class Migration(migrations.Migration):
    dependencies = [("accounts", "0022_seed_ioncourt")]
