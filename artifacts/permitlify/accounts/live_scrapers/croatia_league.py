...
Ports the production ``croatia_league`` spider onto MatchMiner's shared HTTP
client (:mod:`accounts.live_scrapers._http`) + telemetry. The real-time start
form collects **either** a tournament URL **or** a date window
(``input_kind = date_range_or_url``):

* **tournament URL** — scrape that single league directly;
...
Brazil port's choice. ``run(run_obj, log)`` returns ``(items_csv, requests_csv,
errors_csv, row_count, status)``.
"""
...
SEARCH_URL = f"{BASE}/find/league/DoSearch"
COOKIEWALL_URL = f"{BASE}/cookiewall?returnurl=%2Ftournament%2F&lcid=2057"
COOKIEWALL_SAVE_URL = f"{BASE}/cookiewall/Save"
...
# Discovery
# ======================================================================
def _discover_one(client, tournament_url, log):
    """Resolve a single tournament URL to ``[{name, url}]`` (or ``[]``)."""
    ...
    url = urljoin(BASE + "/", href) if href else tournament_url
    if not name:
        name = "Croatia League"
    return [{"tournament_name": name, "tournament_url": url}]


def _discover_range(client, start_date, end_date, log):
...
def _parse_player(client, name, url):
    ...
    if profile_href:
        profile_sel = client.get_selector(urljoin(BASE + "/", profile_href))
        if profile_sel is not None:
            dob = _parse_dob(profile_sel)
    return name, third_party_id, dob, ""


def _build_row(client, ctx, match_data):
    ...
    w1_name, w1_id, w1_dob, w1_g = _parse_player(client, w1.get("name", ""), w1.get("profile_url", ""))
    w2_name, w2_id, w2_dob, w2_g = _parse_player(client, w2.get("name", ""), w2.get("profile_url", ""))
    l1_name, l1_id, l1_dob, l1_g = _parse_player(client, l1.get("name", ""), l1.get("profile_url", ""))
    l2_name, l2_id, l2_dob, l2_g = _parse_player(client, l2.get("name", ""), l2.get("profile_url", ""))
    ...
    return {
        ...
        "winner_1_city": "",
        "winner_1_state": "",
        "winner_1_country": COUNTRY if w1_name else "",
        ...
        "winner_2_city": "",
        "winner_2_state": "",
        "winner_2_country": COUNTRY if w2_name else "",
        ...
        "loser_1_city": "",
        "loser_1_state": "",
        "loser_1_country": COUNTRY if l1_name else "",
        ...
        "loser_2_city": "",
        "loser_2_state": "",
        "loser_2_country": COUNTRY if l2_name else "",
        ...
        "tournament_sanction_body": SANCTION_BODY,
        "winner_2_college": "",
        "loser_2_college": "",
...
def _parse_matches(client, ctx, match_url):
    ...
    for d1 in sel.xpath(
        ...
        if not body:
            continue
        match_data = _parse_match(Selector(text=body))
        if match_data:
            rows.append(_build_row(client, match_ctx, match_data))
    return rows


def _enumerate_group(client, ctx, group_url):
    ...
    for divider in sel.xpath(
        ...
        for d2 in divider.xpath(
            ...
            if not href:
                continue
            items.append((urljoin(BASE + "/", href.strip()), group_ctx))
    return items


def _enumerate_tournament(client, tournament, fallback_year=None):
    ...
    for draw_res in draw_list:
        ...
        if group_id:
            items.extend(
                _enumerate_group(client, ctx, f"{tournament_url}/draw/{group_id}")
            )
    return items
...
def run(run_obj, log):
    """Execute the Croatia League scrape. Returns the standard 5-tuple.
    ...
    if tournament_url:
        log("INFO", "\U0001f3be Croatia League starting \u2014 single tournament URL")
    else:
        start_date, end_date = _window(run_obj)
        log(
            "INFO",
            f"\U0001f3be Croatia League starting \u2014 {start_date} \u2192 {end_date}",
        )
    ...
    with ScraperClient(log=log, tele=tele, proxies=proxies) as discovery:
        ...
        if tournament_url:
            ...
            tournaments = _discover_range(discovery, start_date, end_date, log)
    ...
    def enumerate_one(tournament):
        try:
            return _enumerate_tournament(client_for(), tournament, fallback_year)
    ...
    def crawl_one(item):
        ...
        try:
            rows = _parse_matches(client_for(), group_ctx, match_url)
            for row in rows:
                ...
                log(
                    ...
                    f"@ {row.get('tournament_name') or 'Croatia'}",
                )
