"""Standalone Billie Jean King Cup (ITF Fed Cup) scraper.

Ported from the user's production ``billiejeankingcup`` spider so it runs inside
MatchMiner using only the Python standard library (``urllib``) — no proxies,
``curl_cffi``, ``rich``, ``pandas`` or the original "Atlas" framework. It hits
the public ITF / Stadion data API directly and produces an ITF-style CSV plus a
verbose, streamed log.

A run is intentionally bounded (``MAX_TIES``) so it finishes in a reasonable
time inside the lab. The ``log`` argument is a callback ``log(level, message)``
supplied by the ``run_scrape`` command; every call is persisted as a log line.
"""
...
def _get_json(url, log, tries=3, timeout=25):
    for attempt in range(1, tries + 1):
        try:
            ...
            log("WARN", f"GET {url} -> {exc.__class__.__name__}: {exc} (try {attempt}/{tries})")
...
def _build_row(tie_id, tie_date, match_id, score, log):
    link = f"{API}/match/{match_id}?include={_MATCH_INCLUDE}"
    results = _get_json(link, log)
    if not results:
        return None
    ...
    row.update({
        ...
        "tournament_url": f"https://www.billiejeankingcup.com/en/tie/{tie_id}",
        "outcome": "",
    })
...
def run(run_obj, log):
    """Execute the BJK scrape. Returns ``(csv_text, row_count, status)``."""
    years = _years(run_obj)
    log("INFO", f"Billie Jean King Cup scraper starting \u2014 rank years={years}")
    log("INFO", "HTTP client: urllib (direct, no proxy)")

    ties = []
    seen = set()
    for year in years:
        index_link = f"{API}/custom/wcotDrawsModeled/bjkc/{year}"
        log("INFO", f"GET {index_link}")
        data = _get_json(index_link, log)
        if not data:
    ...
    for idx, (tie_id, tie_date) in enumerate(ties, 1):
        ...
        tie_centre = _get_json(tc_link, log)
        if not tie_centre:
            continue
        ...
        for match in matches:
            ...
            row = _build_row(tie_id, tie_date, match_id, score, log)
            if not row:
                continue
            ...
            log(
                ...
                f"@ {row.get('tournament_name') or 'BJK Cup'}",
            )

    log("INFO", f"Writing {row_count} row(s) to CSV")
    status = Run.Status.SUCCESS if row_count else Run.Status.PARTIAL
    log("INFO", f"Run finished \u2014 status={status}, rows={row_count}")
    return buf.getvalue(), row_count, status
