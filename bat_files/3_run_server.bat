...
echo.
REM  --threads raises the request worker pool above Waitress' small default so
REM  slow clients/downloads do not block every page. --channel-timeout raises the
REM  per-connection inactivity limit (default 120s)
...
"%VENV_PY%" -m waitress --listen=0.0.0.0:80 --threads=16 --channel-timeout=1200 matchminer.wsgi:application
