...
- **Authenticated app** (shares `app_base.html`, DB-backed):
  ...
  - **Scraper Lab** (`/scrapers/<slug>/?tab=…`) — the core feature, tabbed:
    ...
    - **Schedule** — docs-only guide to schedule via **GitHub Actions**: masked trigger URL + token (reveal/copy), numbered setup steps, copy-ready workflow YAML, curl example, and a Regenerate-token button. The webhook `POST /scrapers/<slug>/trigger/` (`@csrf_exempt`) auths via `Authorization: Bearer <token>` (constant-time compare, never logged) and launches the same `_start_scraper_run` helper. Statuses: 401 / 400 / 409 / 503 / 201. **Daily-advance contract:** for `date_range` scrapers the generated YAML posts a **blank** date window (`default: ""`), so each cron run hits the webhook's server-side both-dates-blank fallback and gets a fresh trailing window ending "today" — baking concrete dates would freeze every scheduled run on the same window. Manual `workflow_dispatch` can still type explicit dates.
