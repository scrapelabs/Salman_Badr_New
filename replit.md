...
- `artifacts/permitlify/` — the Django project, previewPath `/`.
  ...
  - `accounts/` — the app:
    ...
    - `live_scrapers/` — real scrapers + their telemetry. `_stadion.py` is the shared, parameterised stdlib-`urllib` port of the production ITF/Stadion team-competition spider (direct or via the scraper's proxy; processes **all** discovered ties concurrently with a `ThreadPoolExecutor` (`MAX_WORKERS=8`) — **no per-run tie cap**, so a full season is scraped (e.g. BJK Cup 2026 ≈ 478 rows) in ~1.5 min, well inside the 20-min reaper; 60-col Title-cased ITF CSV header; `sanitize_cell` injection guard). It exposes `StadionConfig` + `run(config, run_obj, log)` returning the 5-tuple `(items_csv, requests_csv, errors_csv, row_count, status)`. `_build_opener(scraper, log)` honours `run_obj.scraper.proxy`: a `urllib` `ProxyHandler` opener **only** when the proxy is active with a non-empty address (scheme defaulted to `http://`); otherwise an explicit empty `ProxyHandler({})` (direct, ignoring env proxy vars). The opener is threaded through `_get_json`/`_build_row`. The proxy **address is never logged** (may carry credentials) — only its name/type. `billiejeankingcup.py` (Fed Cup, draw code `bjkc`, women) is a thin wrapper: a `CONFIG` + `def run(run_obj, log): return _stadion.run(CONFIG, run_obj, log)`. `_stadion.py` stays parameterised (`StadionConfig` — draw code / gender / match URL), so sibling ITF/Stadion team competitions (e.g. Davis Cup, draw code `dc`, men) can be re-added later as more thin wrappers. `telemetry.py` — `Telemetry` (lock-guarded, since ties are scraped concurrently) records every HTTP request (`record_request`) and error (`record_error`) and emits `requests_csv()` / `errors_csv()` whose columns/formats match the user's production framework (requests: `request_id`=uuid4, `duration`="{ms:.0f} ms", `fingerprint`=sha1(url+status+size), `http_method`, `response_size`="{n} bytes", `http_status`, `last_seen`="%Y-%m-%d %H:%M:%S UTC", `url`; errors: `timestamp`="%Y-%m-%d %H:%M:%S.%f", `level`, `message`, `exception`=traceback). Add new real scrapers here and register them in `run_scrape`'s `LIVE_SCRAPERS`. Real years are derived from `Run.date_from`/`date_to` (the year dropdown sets Jan 1–Dec 31 of one year).
...
The catalogue was deliberately trimmed to a **single** wired scraper — **Billie Jean King Cup** — to perfect it before adding more; other sources will be re-added later. It uses one public ITF/Stadion JSON API needing no proxies/credentials. The shared `_stadion.py` engine is still parameterised (`StadionConfig`), so sibling ITF/Stadion team competitions (e.g. Davis Cup) can be re-added as thin wrappers; other source types would need the original framework's proxy pool / `curl_cffi` / Selenium / AI extraction, which isn't available in this environment. Any slug with no `LIVE_SCRAPERS` entry **fails honestly** (no fabricated data). Assigning a proxy (Settings tab) routes a scraper's HTTP traffic through that endpoint when an address is set, but it does **not** make an unwired scraper work — only a registered `LIVE_SCRAPERS` entry does.
