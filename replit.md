...
- **Authenticated app** (shares `app_base.html`, DB-backed):
  ...
  - **Settings** (`/settings/`, superuser-only, in the Workspace nav group) — merged workspace-config page with three sections: **General** (workspace-wide Anthropic API key — `GeneralConfig` singleton, never logged; status text is masked, but the superuser-only key input prefills the raw value behind a Reveal/Hide toggle so an admin can view/edit it, like the per-scraper Claude key), **Proxy pools** (add/**edit**/delete pools; addresses always rendered via `display_address` (masked) in the listing; per-scraper selection lives in the Lab's Settings tab. Editing a pool opens a modal that fetches the RAW address on demand from a superuser-only JSON endpoint `settings/proxy/<id>/` (`proxy_detail`) — the one intentional place the raw address is exposed, never logged; the modal's address field has a Reveal/Hide toggle and Save is disabled until the fetch succeeds so a failed load can't blank the stored creds), and **Passwords** (per-user change-password modal, Django validation). The General section's Anthropic key input also has a Reveal/Hide toggle (shared `.settings-reveal` pattern). The old standalone `/proxies/` route now 302-redirects here. The Anthropic key resolves per-scraper `Scraper.claude_api_key` → `GeneralConfig` → env `settings.CLAUDE_KEYS`.
...
- `college_dual_match` now renders **every** HTML page through a headless patchright Chromium (full JS, scraper's proxy) as the **primary** fetch — the rendered DOM is what reaches Claude (the parser); curl_cffi is only the fallback (plus PDF/CSV/API). Renders serialize on one owner thread (Playwright sync API is thread-bound); each URL gets up to 3 render attempts (final on a relaunched context) then an honest curl fallback, never fabricated rows. The spec is `uses_browser=True` (browser-exclusivity guard applies; no migration needed). See the `college-browser-fallback` memory.
