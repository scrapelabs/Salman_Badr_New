...
The single Postgres `DATABASE_URL` is owned by the Django app (`artifacts/matchminer`). Schema changes happen **only** via `python3 manage.py migrate`.
...
**How to apply:**
- Keep `scripts/post-merge.sh` on the Django path: `pnpm install --frozen-lockfile` then `cd artifacts/matchminer && python3 manage.py migrate --noinput`. Never reintroduce `pnpm --filter db push` there.
