...
- **Brand tokens live in `:root`** in `artifacts/matchminer/static/css/styles.css` (the short mockup names: `--pr`, `--pr2`, `--ac`, `--ac2`, `--grad`, `--tx`/`--tx2`/`--tx3`, `--sur2`, etc.). Every mockup page shares this same `:root` block, so define once and reuse.
...
The app started as "Permitlify" (building-permit leads) and was rebranded to **MatchMiner**, a tennis intelligence platform. Wordmark split is "Match" + "Miner". The artifact directory has been renamed to `artifacts/matchminer`; older supplied mockups still use dailypermit/permit naming.
